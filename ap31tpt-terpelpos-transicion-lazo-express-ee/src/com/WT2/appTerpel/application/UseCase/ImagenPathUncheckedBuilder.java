 ...
 * @author USUARIO
 */
public class ImagenPathUncheckedBuilder implements IImagePathBuilder{
    ...
    public ImagePath build() {
        ...
        imagaPath.setSufix(PathFile.UNCHECKEDSUFIX);
    ...
    public void valuePath(String path){
        File fullPath = new File(PathFile.IMAGEFOLDER +path+PathFile.UNCHECKEDSUFIX+FileExtension.PNG);
        if( path != null && !path.equals("") && fullPath.exists() ){
            this.fullPathName =fullPath.getPath();
        }else{
            this.fullPathName = PathFile.DEFAULFILE;
        }
    }
}

...
public class TurnosInformeViewController extends javax.swing.JDialog {
    ...
    public TurnosInformeViewController(InfoViewController vistaPrincipal, boolean modal) {
    ...
    private void initComponents() {
        ...
        jLabel19.setBounds(280, 270, 370, 50);
        ...
        jLabel4.setText("INFORME DE TURNOS");
        jPanel1.add(jLabel4);
    ...
    void cargarInformacion() {
        ...
        showMessage("CARGANDO INFORMACIÓN TURNO... ",
                ...
                true, salir, false, LetterCase.FIRST_UPPER_CASE);
                ...
                if (content.length() > 0) {
                    ...
                    for (JsonElement elemento : registros) {
                        ...
                        if (registro.has("id") && registro.get("id").getAsLong() == id
                            && registro.has("report_type") && registro.get("report_type").getAsString().equals("INFORMACION_TURNO")) {
                            return true;
                        }
                    }
                }
    ...
    private synchronized void guardarRegistroPendiente(long id, String reportType) {
        try {
            ...
            Logger.getLogger(TurnosInformeViewController.class.getName()).log(Level.SEVERE, null, e);
        }
    }
    ...
    private void imprimirReporte() {
        ...
        if (existeEnColaPendiente(identificadorJornada)) {
            NovusUtils.printLn("El registro ya está en cola de impresión - ID: " + identificadorJornada);
            return;
        }
            ...
            try {
                ...
                if (!healthResult.tieneRespuesta() || !healthResult.esSaludable()) {
                    ...
                    javax.swing.SwingUtilities.invokeLater(() -> {
                        ...
                        // Usar mostrarPanelMensaje que cierra automáticamente después de 3 segundos
                ...
                guardarRegistroPendiente(identificadorJornada, "INFORMACION_TURNO");
                
                NovusUtils.printLn("========================================");
                NovusUtils.printLn("[TurnosInformeViewController] INICIANDO IMPRESION EN THREAD SEPARADO");
                NovusUtils.printLn("========================================");
                
                jLabel26.setEnabled(false);
                
                // Ejecutar impresión en thread separado para NO BLOQUEAR la UI
                Thread impresionThread = new Thread(() -> {
            NovusUtils.printLn("[Thread-Impresion] Thread de impresión iniciado - NO bloquea UI");
            try {
                ...
                com.application.useCases.shiftReports.PrintShiftInformationRemoteUseCase useCase = 
                    new com.application.useCases.shiftReports.PrintShiftInformationRemoteUseCase(
                        identificadorJornada,
                        identificadorPromotor
                    );
                
                NovusUtils.printLn("[Thread-Impresion] UseCase creado, ejecutando...");
                ...
                javax.swing.SwingUtilities.invokeLater(() -> {
                    try {
                        if (result.isSuccess()) {
                            ...
                            eliminarRegistroPendiente(identificadorJornada, "INFORMACION_TURNO");
                            showMessage("OCURRIO UN ERROR EN LA IMPRESION",
                                    "/com/firefuel/resources/btBad.png",
                                    true, () -> {
                                        NovusUtils.printLn("[Handler] Cerrando panel de error y habilitando botones");
                                        jLabel26.setEnabled(true);
                                        desbloquearBotonImprimir();
                                        // Ocultar el panel de mensaje para permitir interacción
                                        try {
                                            CardLayout layout = (CardLayout) pnlPrincipal.getLayout();
                                            layout.first(pnlPrincipal);
                                        } catch (Exception ex) {
                                            NovusUtils.printLn("[Handler] Error al cerrar panel: " + ex.getMessage());
                                        }
                                    },
                                    true, LetterCase.FIRST_UPPER_CASE);
                        }
                        NovusUtils.printLn("[Thread-Impresion] showMessage completado");
                    } catch (Exception ex) {
                        NovusUtils.printLn("[Thread-Impresion] ERROR en showMessage: " + ex.getMessage());
                        ex.printStackTrace();
                    }
                });
            } catch (Exception e) {
                ...
                eliminarRegistroPendiente(identificadorJornada, "INFORMACION_TURNO");
                javax.swing.SwingUtilities.invokeLater(() -> {
                    showMessage("ERROR AL IMPRIMIR REPORTE",
                            "/com/firefuel/resources/btBad.png",
                            true, () -> {
                                NovusUtils.printLn("[Handler] Cerrando panel de error y habilitando botones");
                                jLabel26.setEnabled(true);
        ...
        // Configurar como daemon thread - NO bloquea cierre de aplicación
        impresionThread.setDaemon(true);
        impresionThread.setName("Thread-Impresion-Reporte");
        impresionThread.start();
        NovusUtils.printLn("[TurnosInformeViewController] Thread de impresión LANZADO - UI sigue responsiva");
                
            } catch (Exception e) {
                Logger.getLogger(TurnosInformeViewController.class.getName()).log(Level.SEVERE, null, e);
                javax.swing.SwingUtilities.invokeLater(() -> {
                    // Solo eliminar si se guardó el registro (después del health check)
                    eliminarRegistroPendiente(identificadorJornada, "INFORMACION_TURNO");
                    desbloquearBotonImprimir();
                    jLabel26.setEnabled(true);
                });
    ...
    class AsyncTask extends Thread {
        ...
        TurnosInformeViewController vista;

        public AsyncTask(TurnosInformeViewController vista) {
            this.vista = vista;
        }
        ...
        void consultarInformeTurno() {
            this.setBloqueoVista(true);
            long inicioRequest = new Date().getTime();
            JsonObject response = null;
            String url = NovusConstante.SECURE_CENTRAL_POINT_EMPLEADOS_JORNADAS_CONSULTA;
            JsonObject jpromo = new JsonObject();
            if (Main.persona != null) {
                jpromo.addProperty("identificadorPromotor", Main.persona.getId());
                ClientWSAsync async = new ClientWSAsync("CONSULTA DE INFORME TURNO", url, NovusConstante.POST, jpromo,
                        true, false);
                try {
                ...
                if (response != null && response.get("data") != null && !response.get("data").isJsonNull()) {
                    ...
                    this.setBloqueoVista(false);
                    jLabel18.setText(Main.persona.getNombre());
                    jLabel22.setText(Main.persona.getGrupoJornadaId() + "");
                    jLabel19.setText(sdfAM.format(Main.persona.getFechaInicio()));
                    showMessage("NO SE HA PODIDO CONSULTAR LA INFORMACION DEL TURNO, POR FAVOR REINTENTA", "/com/firefuel/resources/btBad.png", true, () -> {
                        cerrar();
                    }, true, LetterCase.FIRST_UPPER_CASE);
                }

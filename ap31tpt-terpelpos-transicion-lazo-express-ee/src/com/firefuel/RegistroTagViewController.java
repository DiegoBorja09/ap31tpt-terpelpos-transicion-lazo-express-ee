...
public class RegistroTagViewController extends javax.swing.JDialog {
    ...
    public static RegistroTagViewController instance = null;
    private ObtenerInfoSurtidoresEstacionUseCase obtenerInfoSurtidoresEstacionUseCase;

    public RegistroTagViewController(InfoViewController parent, boolean modal, UsuariosRegistradosViewController users) {
        super(parent, modal);
        this.parent = parent;
        this.userView = users;
        initComponents();
    ...
    public static RegistroTagViewController getInstance(InfoViewController parent, boolean modal, UsuariosRegistradosViewController users) {
        if (RegistroTagViewController.instance == null) {
            RegistroTagViewController.instance = new RegistroTagViewController(parent, modal, users);
        } else {
            RegistroTagViewController.instance.userView = users;
        }
        return RegistroTagViewController.instance;
    }
    ...
    private void initComponents() {
        ...
        jNotificacion.setFont(new java.awt.Font("Arial", 0, 36)); // NOI18N
        ...
        juser.setBounds(70, 190, 510, 60);
        pnl_principal.add(jPanel1);
        jPanel1.setBounds(650, 170, 570, 470);

        jLabel7.setFont(new java.awt.Font("Terpel Sans", 1, 36)); // NOI18N
        jLabel7.setForeground(new java.awt.Color(255, 255, 255));
        jLabel7.setText("REGISTRO TAG");
        pnl_principal.add(jLabel7);
        jLabel7.setBounds(100, 0, 750, 80);

        jLabel3.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/firefuel/resources/presentar_rfid.png"))); // NOI18N
        pnl_principal.add(jLabel3);
        jLabel3.setBounds(150, 270, 330, 270);
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/firefuel/resources/fndTagfid.png"))); // NOI18N
  ...
  private void jtagActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jtagActionPerformed
    ...
    } // GEN-FIRST:event_jLabel2MousePressed // GEN-LAST:event_jLabel2MousePressed
    ...
    private void cerrar() {
        ...
        InfoViewController.instanciaRegistroTag = null;
        RegistroTagViewController.instance = null;
    ...
    public void guardar() {
        ...
        if (identificacion.equals("")) {
            ...
            mostrarPanelMensaje("POR FAVOR ACERQUE EL TAG RFID",
                    "/com/firefuel/resources/btBad.png",
                    LetterCase.FIRST_UPPER_CASE);
        } else {
            long idPersonaRegistrada = RegistroTagViewController.guardarIdentificacion(identificacion, tag);
            if (idPersonaRegistrada == 0) {
                ...
                mostrarPanelMensaje("TAG RFID ASIGNADO CORRECTAMENTE",
                        "/com/firefuel/resources/btOk.png",
                        LetterCase.FIRST_UPPER_CASE);
                this.userView.solicitarUsuarios();
                this.userView.renderizarUsuarios();
            }
        }
    }
    ...
    public void notifyAllDevices(String identificacion, String tag) {
        ...
        for (JsonElement element : infoPosEstacion) {
            ...
            if (!(Utils.getIpLocal().trim().equals(host))) {
                try {
                    edao.guardarTransmision(Main.credencial, "{ \"medio\": \"rfid\",\n" + "    \"identificacion\": \"" + identificacion + "\",\n" + "    \"tag\": \"" + tag + "\"}", url, NovusConstante.POST);
                } catch (DAOException ex) {
                    Logger.getLogger(RegistroTagViewController.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
    }
    ...
    private void sendRegistro(long idPersona, String tag) {
        ...
        EquipoDao edao = new EquipoDao();
        try {
            edao.guardarTransmision(Main.credencial, json.toString(), NovusConstante.getServer(NovusConstante.SECURE_END_POINT_ASIGNACION_TAG), NovusConstante.POST);
        } catch (DAOException ex) {
            Logger.getLogger(RegistroTagViewController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void Async(Runnable runnable, int tiempo) {
        new Thread(() -> {
            try {
                ...
                Logger.getLogger(RegistroTagViewController.class.getName()).log(Level.SEVERE, null, e);

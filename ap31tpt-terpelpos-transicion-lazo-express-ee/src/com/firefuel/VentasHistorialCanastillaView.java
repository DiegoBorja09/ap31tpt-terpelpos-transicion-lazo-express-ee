...
import com.application.useCases.movimientos.ObtenerVentasCanastillaUseCase;
import com.application.useCases.printService.CheckPrintServiceHealthUseCase;

public class VentasHistorialCanastillaView extends JDialog {
    ...
    ArrayList<MovimientosBean> listaVentasCanastilla = new ArrayList<>();
    ...
    public static int NUMERO_IMPRIMIR;
    MovimientosDao mdao = new MovimientosDao();
    ObtenerConsecutivoUseCase obtenerConsecutivoUseCaseFE;
    ObtenerConsecutivoUseCase obtenerConsecutivoUseCaseNoFE;
    ...
    private final Icon iconoBotonActivo = new javax.swing.ImageIcon(getClass().getResource("/com/firefuel/resources/botones/bt-danger-small.png"));
    private final Icon iconoBotonBloqueado = new javax.swing.ImageIcon(getClass().getResource("/com/firefuel/resources/botones/bt-link-small.png"));
    ...
    public VentasHistorialCanastillaView(InfoViewController parent, boolean modal) {
        super(parent, modal);
        this.parent = parent;
        this.obtenerConsecutivoUseCaseFE = new ObtenerConsecutivoUseCase(true, "CAN");
        this.obtenerConsecutivoUseCaseNoFE = new ObtenerConsecutivoUseCase(false, "CAN");
        initComponents();
        this.init();
    }
    ...
    void init() {
        ...
        timerVerificarCola = new javax.swing.Timer(500, e -> actualizarEstadoBotonImprimir());
    ...
    private void initComponents() {
        ...
        jLabel1.setBounds(830, 100, 190, 70);
        ...
        jLabel4.setBounds(1030, 100, 180, 70);

        jLabel3.setFont(new java.awt.Font("Terpel Sans", 1, 36)); // NOI18N
        jLabel3.setForeground(new java.awt.Color(255, 255, 255));
        jLabel3.setText("HISTORIAL CANASTILLA");
        ...
        pnl_confirmacion.add(jLabel6);
        jLabel6.setBounds(307, 250, 690, 180);
        ...
        SI1.setBounds(670, 480, 170, 56);

        jLabel9.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel9.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/firefuel/resources/card.png"))); // NOI18N
        jLabel9.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        pnl_confirmacion.add(jLabel9);
        jLabel9.setBounds(250, 210, 800, 360);
    ...
    private void jLabel8MouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel8MouseReleased
        if (jLabel8.isEnabled()) {
            ...
            jLabel6.setText("<html><center>¿Desea generar la devolución de esta venta?</center></html>");
            handler = () -> {
                enviarNotaCredito();
            };
        }
    ...
    public void refrescar(String accion) {
        setTimeout(0, () -> {
            ...
            try {
                ...
                int i = 0;
                for (MovimientosBean movimiento : listaVentasCanastilla) {
                    ...
                    try {
                        System.out.println("[LOG] Agregando fila a la tabla: id=" + movimiento.getId() + ", consecutivo=" + (movimiento.getConsecutivo() != null ? movimiento.getConsecutivo().getPrefijo() + "-" + movimiento.getConsecutivo().getConsecutivo_actual() : "null"));
                        
                        // 🔧 FIX: Manejar caso cuando consecutivo es null
                        String consecutivoTexto = "SIN CONSECUTIVO";
                        if (movimiento.getConsecutivo() != null) {
                            consecutivoTexto = movimiento.getConsecutivo().getPrefijo() + "-" + movimiento.getConsecutivo().getConsecutivo_actual();
                        }
                        ...
                        System.out.println("[LOG][ERROR] Error agregando fila a la tabla: " + e.getMessage());
                        e.printStackTrace();
                    }
                }
                // jTable1.setAutoResizeMode(JTable.AUTO_RESIZE_SUBSEQUENT_COLUMNS);
            } catch (Exception s) {
                System.err.println(s + " " + accion);
            }
        });
                ...
                if (movimientoSeleccionado != null) {
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            body.addProperty("tipo_negocio", "CANASTILLA");   // 🔥 Tipo de negocio
                            body.add("body", data);
                            ...
                            ClientWSAsync client = new ClientWSAsync("IMPRESION CANASTILLA", url, NovusConstante.POST, body, true, false, header, 10000);
                            client.start();
                            ...
                            Logger.getLogger(VentasHistorialCanastillaView.class.getName()).log(Level.SEVERE, null, e);
                            javax.swing.SwingUtilities.invokeLater(() -> desbloquearBotonImprimir());
                        }
                    }).start();
                }
    ...
    public void solicitarVentas(Date fechaIncial, Date fechaFinal) {
        if (fechaIncial == null) {
        ...
        if (jCheckBox1.isSelected() && !promotores.isEmpty()) {
            ...
            idPromotor = String.valueOf(identificadorPromotor);
        }
        ...
        ObtenerVentasCanastillaUseCase useCase = new ObtenerVentasCanastillaUseCase(
            sdf1.format(fechaIncial) + " 00:00:00",
            sdf1.format(fechaFinal) + " 23:59:59",
            idPromotor
        );
        JsonObject response = useCase.execute();

        try {
            if (response != null) {
                listaVentasCanastilla.clear();
                ...
                for (JsonElement jsonElement : array) {
                    try {
                        ...
                        if (mov.getAtributos().get("consecutivo") != null && mov.getAtributos().get("consecutivo").isJsonObject()) {
                            ...
                            try {
                                if (json.get("atributos").getAsJsonObject() != null && json.get("atributos").getAsJsonObject().has("isElectronica")) {
                                    boolean isElectronica = json.get("atributos").getAsJsonObject().get("isElectronica").getAsBoolean();
                                    System.out.println("[LOG] Usando caso de uso FE/NoFE - isElectronica: " + isElectronica);
                                    conObtenido = isElectronica ? obtenerConsecutivoUseCaseFE.execute() : obtenerConsecutivoUseCaseNoFE.execute();
                                } else {
                                    System.out.println("[LOG] Usando caso de uso NoFE por defecto");
                                    conObtenido = obtenerConsecutivoUseCaseNoFE.execute();
                                }
                                                                 System.out.println("[LOG] Caso de uso retornó: " + (conObtenido != null ? 
                                     "ID=" + conObtenido.getId() + ", Prefijo=" + conObtenido.getPrefijo() + ", Actual=" + conObtenido.getConsecutivo_actual() : "null"));
                            } catch (Exception ex) {
                                System.out.println("[LOG][ERROR] Error en caso de uso: " + ex.getMessage());
                                Logger.getLogger(VentasHistorialCanastillaView.class.getName()).log(Level.SEVERE, null, ex);
                            }
                            if (conObtenido != null) {
                                System.out.println("[LOG] Sobreescribiendo consecutivo con datos del JSON");
                                ...
                                System.out.println("[LOG] Consecutivo final: " + conObtenido.getPrefijo() + "-" + conObtenido.getConsecutivo_actual());
                            } else {
                                // 🔧 FIX: Crear ConsecutivoBean con datos históricos del JSON cuando el caso de uso retorna null
                                // Esto permite mostrar ventas históricas aunque el consecutivo actual esté vencido
                                System.out.println("[LOG] ConsecutivoBean es null, creando uno con datos históricos del JSON");
                                conObtenido = new ConsecutivoBean();
                                conObtenido.setId(jsonConsecutivo.get("id").getAsLong());
                                conObtenido.setPrefijo(!jsonConsecutivo.get("prefijo").isJsonNull() ? jsonConsecutivo.get("prefijo").getAsString() : "");
                                conObtenido.setConsecutivo_actual(!json.get("consecutivo_venta").isJsonNull() ? json.get("consecutivo_venta").getAsLong() : 0);
                                conObtenido.setObservaciones("");
                                System.out.println("[LOG] Consecutivo histórico creado: " + conObtenido.getPrefijo() + "-" + conObtenido.getConsecutivo_actual());
                            }
                            mov.setConsecutivo(conObtenido);
                        } else {
                            System.out.println("[LOG] No hay consecutivo en el JSON de atributos");
                            // 🔧 FIX: Crear consecutivo por defecto cuando no hay consecutivo en atributos
                            ConsecutivoBean consecutivoDefault = new ConsecutivoBean();
                            consecutivoDefault.setId(0L);
                            consecutivoDefault.setPrefijo("N/A");
                            consecutivoDefault.setConsecutivo_actual(0L);
                            consecutivoDefault.setObservaciones("Sin consecutivo");
                            mov.setConsecutivo(consecutivoDefault);
                            System.out.println("[LOG] Consecutivo por defecto creado: " + consecutivoDefault.getPrefijo() + "-" + consecutivoDefault.getConsecutivo_actual());
                        }
                        LinkedHashMap<Long, MovimientosDetallesBean> detalles = new LinkedHashMap<>();
                        float totalImpuestos = 0;
                        
                        if (json.has("detalles") && !json.get("detalles").isJsonNull()) {
                            for (JsonElement jsonElementDetalles : json.getAsJsonArray("detalles")) {
                                if (jsonElementDetalles.isJsonNull()) continue;
                                ...
                                JsonObject atributos = jsonDetalles.has("atributos") && !jsonDetalles.get("atributos").isJsonNull() ? 
                                    jsonDetalles.get("atributos").getAsJsonObject() : new JsonObject();
                                    
                                int tipo = atributos.has("tipo") && !atributos.get("tipo").isJsonNull() ? 
                                    atributos.get("tipo").getAsInt() : 0;
                                    
                                String medida = jsonDetalles.has("unidad") && !jsonDetalles.get("unidad").isJsonNull() ? 
                                    jsonDetalles.get("unidad").getAsString() : "";
                                    
                                detalleVenta.setTipo(tipo);
                                detalleVenta.setUnidades_medida(medida);
                                detalleVenta.setCantidadUnidad(jsonDetalles.has("cantidad") && !jsonDetalles.get("cantidad").isJsonNull() ? 
                                    jsonDetalles.get("cantidad").getAsFloat() : 0);
                                detalleVenta.setCantidad(jsonDetalles.has("cantidad") && !jsonDetalles.get("cantidad").isJsonNull() ? 
                                    jsonDetalles.get("cantidad").getAsFloat() : 0);
                                detalleVenta.setPlu(jsonDetalles.has("plu") && !jsonDetalles.get("plu").isJsonNull() ? 
                                    jsonDetalles.get("plu").getAsString() : "");
                                detalleVenta.setCosto(jsonDetalles.has("costo_producto") && !jsonDetalles.get("costo_producto").isJsonNull() ? 
                                    jsonDetalles.get("costo_producto").getAsFloat() : 0);
                                detalleVenta.setPrecio(jsonDetalles.has("precio") && !jsonDetalles.get("precio").isJsonNull() ? 
                                    jsonDetalles.get("precio").getAsFloat() : 0);
                                detalleVenta.setSubtotal(jsonDetalles.has("sub_total") && !jsonDetalles.get("sub_total").isJsonNull() ? 
                                    jsonDetalles.get("sub_total").getAsFloat() : 0);
                                detalleVenta.setId(jsonDetalles.has("id_producto") && !jsonDetalles.get("id_producto").isJsonNull() ? 
                                    jsonDetalles.get("id_producto").getAsLong() : 0);
                                detalleVenta.setDescripcion(jsonDetalles.has("nombre_producto") && !jsonDetalles.get("nombre_producto").isJsonNull() ? 
                                    jsonDetalles.get("nombre_producto").getAsString() : "");
                                detalleVenta.setBodegasId(jsonDetalles.has("bodega_id") && !jsonDetalles.get("bodega_id").isJsonNull() ? 
                                    jsonDetalles.get("bodega_id").getAsLong() : 0);
                                
                                if (jsonDetalles.has("atributos") && !jsonDetalles.get("atributos").isJsonNull()) {
                                    detalleVenta.setAtributos(jsonDetalles.get("atributos").getAsJsonObject());
                                }
                                
                                ArrayList<ImpuestosBean> impuestos = new ArrayList<>();
                                if (jsonDetalles.has("impuestos") && !jsonDetalles.get("impuestos").isJsonNull()) {
                                    for (JsonElement jsonElementImpuestos : jsonDetalles.getAsJsonArray("impuestos")) {
                                        if (jsonElementImpuestos.isJsonNull()) continue;
                                        
                                        JsonObject jsonImpuestos = jsonElementImpuestos.getAsJsonObject();
                                        ImpuestosBean impuesto = new ImpuestosBean();
                                        impuesto.setId(jsonImpuestos.has("id_impuesto") && !jsonImpuestos.get("id_impuesto").isJsonNull() ? 
                                            jsonImpuestos.get("id_impuesto").getAsLong() : 0);
                                        impuesto.setDescripcion(jsonImpuestos.has("nombre_impuesto") && !jsonImpuestos.get("nombre_impuesto").isJsonNull() ? 
                                            jsonImpuestos.get("nombre_impuesto").getAsString() : "");
                                        impuesto.setPorcentaje_valor(jsonImpuestos.has("porcentaje_valor") && !jsonImpuestos.get("porcentaje_valor").isJsonNull() ? 
                                            jsonImpuestos.get("porcentaje_valor").getAsString() : "");
                                        impuesto.setCalculado(jsonImpuestos.has("impuesto_valor") && !jsonImpuestos.get("impuesto_valor").isJsonNull() ? 
                                            jsonImpuestos.get("impuesto_valor").getAsFloat() : 0);
                                        impuesto.setValor(jsonImpuestos.has("valor") && !jsonImpuestos.get("valor").isJsonNull() ? 
                                            jsonImpuestos.get("valor").getAsFloat() : 0);
                        ...
                        if (json.has("medios_pagos") && !json.get("medios_pagos").isJsonNull()) {
                            for (JsonElement jsonElementMedios : json.getAsJsonArray("medios_pagos")) {
                                if (jsonElementMedios.isJsonNull()) continue;
                                
                                JsonObject jsonMedio = jsonElementMedios.getAsJsonObject();
                                MediosPagosBean medio = new MediosPagosBean();
                                medio.setId(jsonMedio.has("id_medio_de_pago") && !jsonMedio.get("id_medio_de_pago").isJsonNull() ? 
                                    jsonMedio.get("id_medio_de_pago").getAsLong() : 0);
                                medio.setDescripcion(jsonMedio.has("nombre_medio_de_pago") && !jsonMedio.get("nombre_medio_de_pago").isJsonNull() ? 
                                    jsonMedio.get("nombre_medio_de_pago").getAsString() : "");
                                medio.setCambio(jsonMedio.has("valor_cambio") && !jsonMedio.get("valor_cambio").isJsonNull() ? 
                                    jsonMedio.get("valor_cambio").getAsFloat() : 0);
                                medio.setValor(jsonMedio.has("valor_total") && !jsonMedio.get("valor_total").isJsonNull() ? 
                                    jsonMedio.get("valor_total").getAsFloat() : 0);
                                medio.setRecibido(jsonMedio.has("valor_recibido") && !jsonMedio.get("valor_recibido").isJsonNull() ? 
                                    jsonMedio.get("valor_recibido").getAsFloat() : 0);
                                medio.setVoucher(jsonMedio.has("comprobante") && !jsonMedio.get("comprobante").isJsonNull() ? 
                                    jsonMedio.get("comprobante").getAsString() : "");
                                medio.setComprobante(jsonMedio.has("tiene_comprobante") && !jsonMedio.get("tiene_comprobante").isJsonNull() ? 
                                    jsonMedio.get("tiene_comprobante").getAsString().equals("S") : false);
    ...
    private void generarNotaProducto(JsonArray dataNota, int numeroVenta) {
        NotasCredito nota = new NotasCredito();
        respuestaNota = nota.dataproductoNota(dataNota, numeroVenta, jLabel8);
    }
    ...
    private JsonObject imprimirfacFE(JsonArray datanota, int numero) {
        FacturacionElectronica fac = new FacturacionElectronica();
        JsonObject data = fac.imprimirFE(datanota, numero);

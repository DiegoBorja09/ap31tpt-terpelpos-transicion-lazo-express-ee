...
/**
 * Use Case: Imprimir Consolidado de Turno usando Print Ticket Service (Python)
 * 
 * Arquitectura Limpia: Use Case que encapsula la lógica de negocio
 * para enviar solicitudes de impresión de reportes consolidados (Opción 5)
 * al microservicio Python.
 * 
 * Reemplaza: PrinterFacade.printJornadaActual() para Opción 5
 * 
 * @author Equipo Desarrollo
 * @version 1.0
 */
public class PrintShiftConsolidatedRemoteUseCase implements BaseUseCasesWithParams<Void, ShiftReportResult> {
    ...
    /**
     * Constructor para impresión de consolidado de turno
     * 
     ...
     */
    public PrintShiftConsolidatedRemoteUseCase(long identificadorJornada, long identificadorPromotor, int copies) {
        this.identificadorJornada = identificadorJornada;
        this.identificadorPromotor = identificadorPromotor;
        this.copies = copies;
        this.endpoint = NovusConstante.PRINT_TICKET_SHIFT_CONSOLIDATED;
    }
    
    /**
     * Constructor simplificado con 1 copia por defecto
     */
    public PrintShiftConsolidatedRemoteUseCase(long identificadorJornada, long identificadorPromotor) {
    ...
    public ShiftReportResult execute(Void input) {
        try {
            ...
            // 2. Log de inicio
            NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] Enviando solicitud a: " + endpoint);
            NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] Payload: " + payload.toString());
            
            // 3. Configurar headers HTTP
            TreeMap<String, String> headers = new TreeMap<>();
            headers.put("Content-Type", "application/json");
            
            // 4. Crear cliente HTTP asíncrono con timeout
            ClientWSAsync client = new ClientWSAsync(
                "PRINT_SHIFT_CONSOLIDATED", 
                endpoint, 
                NovusConstante.POST, 
                payload, 
            ...
            try {
                ...
                NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] ERROR: Interrumpido esperando respuesta");
            ...
            if (response != null) {
                ...
                if (response.has("success") && !response.get("success").getAsBoolean()) {
                    ...
                    NovusUtils.printLn("=====================================================");
                    NovusUtils.printLn("ERROR AL IMPRIMIR CONSOLIDADO DE TURNO (OPCION 5)");
                    NovusUtils.printLn("=====================================================");
                ...
                if (response.has("print_job_id") || response.has("success")) {
                    String message = response.has("message") 
                        ? response.get("message").getAsString() 
                        : "Reporte consolidado impreso exitosamente";
                    
                    NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] SUCCESS: " + message);
                    
                    if (response.has("print_job_id")) {
                        String jobId = response.get("print_job_id").getAsString();
                        NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] Job ID: " + jobId);
                    }
                    
                    // Log additional data if available
                    if (response.has("sales_count")) {
                        int salesCount = response.get("sales_count").getAsInt();
                        double salesTotal = response.get("sales_total").getAsDouble();
                        NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] Sales: " + salesCount + " | Total: $" + salesTotal);
                    }
                ...
                NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] WARNING: Respuesta inesperada");
                NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] Response: " + response.toString());
                ...
                if (client.getError() != null) {
                    JsonObject errorResponse = client.getError();
            ...
            NovusUtils.printLn("[PrintShiftConsolidatedRemoteUseCase] ERROR: " + e.getMessage());
            e.printStackTrace();
    ...
    private JsonObject buildPayload() {
        ...
        payload.addProperty("flow_type", "REPORTE_CONSOLIDADO_TURNO");
